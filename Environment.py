...
class Environment():
    ...
    def readConfigFile(self):
        ...
        if self.mainConfigFile == self.mainConfigFile == "":
            raise Exception('Please provide absolute path for config.ini')

        else:
            #print Environment.mainConfigFile
	    config = ConfigParser.ConfigParser()
            config.read(self.mainConfigFile)
	    options = config.options(self.controllerName)
            self.dictionary = dict()
            for option in options:
                self.dictionary[option] = config.get(self.controllerName, option)
	...
	#if '''self.controllerName == "pyretic" or''' self.controllerName == "kinetic":
	if self.controllerName == "pyretic" or self.controllerName == "kinetic" :
		time.sleep(1)
		return process
	...
	'''self.startController()
        ...
        #self.pingalll(mnProcess)
	self.stopApplication()
        self.stopMininet(mnProcess)
        self.stopController()'''
	#print "controller name is "+self.controllerName
	#return
	if self.controllerName == "kinetic" or self.controllerName =="pyretic":
		mnProcess = self.runMininet(self.dictionary)
                print "runmininet and wait"
                time.sleep(2)
                self.startController()
                self.startApplication(self.applicationName)
                print "application started"
                self.pingalll(mnProcess)
                print "pingall command"
                mnProcess = self.runMininet(self.dictionary)
                self.pingalll(mnProcess)
                self.stopApplication()
                print "stopping app"
