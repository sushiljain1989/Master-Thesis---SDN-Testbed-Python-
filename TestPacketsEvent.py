	...
	def result(self):
		...
		for key in timestamps:
			if prev_event == "":
				...
				latestTime = float(key)
				events_dict[startTime] = { "event":prev_event , "duration":latestTime-startTime , "numPkts":pkts }
				startTime = float(key)
				#latestTime = float(key)
				prev_event = pkt_events[key]
				pkts = 1
		...
		return collections.OrderedDict(sorted(events_dict.items(), key=lambda t: t[0]))#events_dict
