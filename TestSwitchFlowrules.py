...
class TestSwitchFlowrules(Test):
	...
	def execute(self, proces = None, config = None):
		...
		for switchInterfaceName in self.switchesList:
			...
			process = subprocess.Popen(["for d in $(seq 1 400); do sudo ovs-ofctl dump-flows "+switchInterfaceName+" >> "+self.config['templogsdir']+switchInterfaceName+".log"+"; sleep 0.1; done;"], shell=True,stdin=subprocess.PIPE , stdout = subprocess.PIPE, stderr = subprocess.STDOUT)
