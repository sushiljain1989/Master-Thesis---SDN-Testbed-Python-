...
public class LearningSwitchFirewall
implements IFloodlightModule, ILearningSwitchService, IOFMessageListener, IControllerCompletionListener {
	protected static Logger log = LoggerFactory.getLogger(LearningSwitchFirewall.class);
	protected int rulesWrite = 0;
	// Module dependencies
	...
	//rningSwitchFirewall Stores the learned state for each switch
	...
	private void writeFlowMod(IOFSwitch sw, OFFlowModCommand command, OFBufferId bufferId,
		...
		fmb.setMatch(match);
		fmb.setCookie((U64.of(LearningSwitchFirewall.LEARNING_SWITCH_COOKIE)));
		fmb.setIdleTimeout(LearningSwitchFirewall.FLOWMOD_DEFAULT_IDLE_TIMEOUT);
		fmb.setHardTimeout(LearningSwitchFirewall.FLOWMOD_DEFAULT_HARD_TIMEOUT);
		fmb.setPriority(LearningSwitchFirewall.FLOWMOD_PRIORITY);
	...
	private Command processFlowRemovedMessage(IOFSwitch sw, OFFlowRemoved flowRemovedMessage) {
		if (!flowRemovedMessage.getCookie().equals(U64.of(LearningSwitchFirewall.LEARNING_SWITCH_COOKIE))) {
			return Command.CONTINUE;
		}
		if (log.isTraceEnabled()) {
