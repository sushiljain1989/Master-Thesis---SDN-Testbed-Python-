...
class VlanApp2(frenetic.App):
  ...
  def policy_for_dest(self, mac_port):
    ...
    return Filter(VlanEq(mac_vlan)) >> Filter(EthDstEq(mac)) >> SetPort(port)

  def policies_for_dest(self, all_mac_ports):
    return [ self.policy_for_dest(mp) for mp in all_mac_ports ]

  def policy(self):
    return IfThenElse(EthDstNotEq( self.nib.all_learned_macs() ),SendToController("vlan_app"),Union( self.policies_for_dest(self.nib.all_mac_port_pairs())))
